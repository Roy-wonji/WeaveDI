...
final class IntegrationUserRepositoryImpl: IntegrationUserService, @unchecked Sendable {
    private var networkClient: IntegrationNetworkClient? {
        return WeaveDI.Container.live.resolve(IntegrationNetworkClient.self)
    }

    private var cacheService: IntegrationCacheService? {
        return WeaveDI.Container.live.resolve(IntegrationCacheService.self)
    }
    ...
    // IntegrationUserService requirements
    func getUser(id: String) async throws -> IntegrationUser {
        guard let user = await findUser(id: id) else {
    ...
    func deleteUser(id: String) async throws {
        await cacheService?.remove("user_\(id)")
    }
}
...
final class IntegrationUserServiceImpl: IntegrationUserService, @unchecked Sendable {
    private var repository: IntegrationUserRepositoryImpl? {
        return WeaveDI.Container.live.resolve(IntegrationUserRepositoryImpl.self)
    }

    private var analytics: IntegrationAnalyticsService? {
        return WeaveDI.Container.live.resolve(IntegrationAnalyticsService.self)
    }
...
final class IntegrationTests: XCTestCase {
    ...
    func testCompleteUserServiceFlow_전체사용자서비스플로우() async throws {
        // Setup dependencies using WeaveDI.Container
        await setupProductionDependencies()

        // Get the main service
        let userService = WeaveDI.Container.live.resolve(IntegrationUserService.self)
        ...
        let analytics = WeaveDI.Container.live.resolve(IntegrationAnalyticsService.self) as? IntegrationAnalyticsServiceImpl
    ...
    func testCacheIntegration_캐시통합() async throws {
        ...
        let repository = WeaveDI.Container.live.resolve(IntegrationUserRepositoryImpl.self)
    ...
    func testErrorHandlingIntegration_에러처리통합() async {
        ...
        // Register services individually with WeaveDI.Container
        WeaveDI.Container.live.register(IntegrationNetworkClient.self, instance: FailingNetworkClient())
        WeaveDI.Container.live.register(IntegrationCacheService.self, instance: IntegrationCacheServiceImpl())
        WeaveDI.Container.live.register(IntegrationAnalyticsService.self, instance: IntegrationAnalyticsServiceImpl())
        WeaveDI.Container.live.register(IntegrationUserRepositoryImpl.self, instance: IntegrationUserRepositoryImpl())
        WeaveDI.Container.live.register(IntegrationUserService.self, instance: IntegrationUserServiceImpl())
    ...
    func testConcurrentUserOperations_동시사용자작업() async throws {
        ...
        XCTAssertEqual(fetchStartedEvents.count, 10)
        XCTAssertEqual(fetchSuccessEvents.count, 10)
    }
    ...
    func testPropertyWrapperIntegration_프로퍼티래퍼통합() async throws {
        // Setup dependencies
        await setupProductionDependencies()

        // Test that container resolution works correctly in integration scenario
        let userService = IntegrationUserServiceImpl()

        // Test actual usage - this internally uses container resolution
        let user = try await userService.getUser(id: "property_wrapper_test")
        XCTAssertEqual(user.id, "property_wrapper_test")
        XCTAssertEqual(user.name, "User property_wrapper_test")

        // Verify dependencies are properly resolved
        let repository = WeaveDI.Container.live.resolve(IntegrationUserRepositoryImpl.self)
        let analytics = WeaveDI.Container.live.resolve(IntegrationAnalyticsService.self)
        XCTAssertNotNil(repository)
    ...
    private func setupProductionDependencies() async {
        WeaveDI.Container.live.register(IntegrationNetworkClient.self, instance: IntegrationNetworkClientImpl())
        WeaveDI.Container.live.register(IntegrationCacheService.self, instance: IntegrationCacheServiceImpl())
        WeaveDI.Container.live.register(IntegrationAnalyticsService.self, instance: IntegrationAnalyticsServiceImpl())
        WeaveDI.Container.live.register(IntegrationUserRepositoryImpl.self, instance: IntegrationUserRepositoryImpl())
        WeaveDI.Container.live.register(IntegrationUserService.self, instance: IntegrationUserServiceImpl())
    ...
    private func setupDevelopmentDependencies() async {
        WeaveDI.Container.live.register(IntegrationNetworkClient.self, instance: MockIntegrationNetworkClient())
        WeaveDI.Container.live.register(IntegrationCacheService.self, instance: IntegrationCacheServiceImpl())
        WeaveDI.Container.live.register(IntegrationAnalyticsService.self, instance: IntegrationAnalyticsServiceImpl())
        WeaveDI.Container.live.register(IntegrationUserRepositoryImpl.self, instance: IntegrationUserRepositoryImpl())
        WeaveDI.Container.live.register(IntegrationUserService.self, instance: IntegrationUserServiceImpl())
...
extension WeaveDI.Container {
    ...
    var integrationUserRepository: IntegrationUserRepositoryImpl? {
        return resolve(IntegrationUserRepositoryImpl.self)
    }
