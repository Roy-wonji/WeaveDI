...
extension DIContainer {
  ...
  /// 컨테이너를 부트스트랩합니다 (비동기 등록, 단일 진입점)
  ///
  /// 앱 시작 시 의존성을 초기화하기 위한 단일 진입점입니다.
  /// 원자적으로 컨테이너를 교체하여 초기화 경합을 방지합니다.
  ///
  /// - Parameter configure: 비동기 의존성 등록 클로저
  static func bootstrap(_ configure: @Sendable (DIContainer) async -> Void) async {
    let newContainer = DIContainer()
    await configure(newContainer)
    Self.shared = newContainer
    DILogger.debug("Container bootstrapped")
  }
