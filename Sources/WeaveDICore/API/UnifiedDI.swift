...
public enum UnifiedDI {
  ...
  /// 단순 부트스트랩 전용 등록 컨텍스트
  ///
  /// 외부에는 register/resolve만 노출하여 Core API를 최소화합니다.
  public struct Bootstrap {
    ...
    public func register<T>(_ instance: T) where T: Sendable {
    ...
    public func resolve<T>() -> T? where T: Sendable {
      resolve(T.self)
    }

    public func resolve<T>(_ type: T.Type) -> T? where T: Sendable {
      container.resolve(type, logOnMiss: false)
    }

    public func requireResolve<T>() -> T where T: Sendable {
      requireResolve(T.self)
    }

    public func requireResolve<T>(_ type: T.Type) -> T where T: Sendable {
      guard let resolved = resolve(type) else {
        fatalError("Required dependency \(T.self) not found (UnifiedDI.Bootstrap)")
      }
      return resolved
    }
  }

  /// Core 기준 단일 부트스트랩 엔트리 (sync)
  ///
  /// register/resolve만 사용하도록 제한된 간단한 초기화 API입니다.
  public static func bootstrap(_ configure: @Sendable (Bootstrap) -> Void) {
    let container = DIContainer.shared
    configure(Bootstrap(container: container))
    DILogger.debug("UnifiedDI bootstrapped")
  }

  /// Core 기준 단일 부트스트랩 엔트리 (async)
  public static func bootstrap(_ configure: @Sendable (Bootstrap) async -> Void) async {
    let container = DIContainer.shared
    await configure(Bootstrap(container: container))
    DILogger.debug("UnifiedDI bootstrapped")
  }
  ...
  ///
  /// DIContainer의 KeyPath를 사용하여 더욱 타입 안전하게 비동기 등록합니다.
  ///
  /// ### 사용 예시:
  /// ```swift
  /// Task {
  ...
  ///
  /// DIContainer의 KeyPath를 사용하여 더욱 타입 안전하게 등록합니다.
  ///
  /// ### 사용 예시:
  /// ```swift
  ...
  public static func resolve<T>(
    ...
    guard let resolved = DIContainer.shared.resolve(type, logOnMiss: logOnMiss) else {
  ...
  ///
  /// - Parameter keyPath: DIContainer 내의 KeyPath
  /// - Returns: 해결된 인스턴스 (없으면 nil)
    public static func resolve<T>(_ keyPath: KeyPath<DIContainer, T?>) -> T? {
  ...
  public static func resolveAsync<T>(_ type: T.Type) async -> T? where T: Sendable {
    let startTime = Date()
    let result = await DIContainer.shared.resolveAsync(type)
    let duration = Date().timeIntervalSince(startTime)
  ...
  public static func resolve<T>(_ type: T.Type, default defaultValue: @autoclosure () -> T) -> T where T: Sendable {
    return DIContainer.shared.resolve(type) ?? defaultValue()
  }
  ...
  public static func release<T>(_ type: T.Type) where T: Sendable {
    DIContainer.shared.release(type)
    FastResolveCache.shared.set(type, value: nil)
  }

  public static func releaseAsync<T>(_ type: T.Type) async where T: Sendable {
    await DIContainer.shared.releaseAsync(type)
    FastResolveCache.shared.set(type, value: nil)
  }

  /// 현재 컨테이너의 자식 컨테이너 개수를 반환합니다.
  public static func childContainerCount() -> Int {
    return DIContainer.shared.getChildren().count
  }
...
extension UnifiedDI {
  ...
  /// }
  /// ```
  static func waitForRegistration() async {
  ...
  static func prewarm<T: Sendable>(_ types: [T.Type]) {
    for type in types {
  ...
  static func performBatchRegistration(_ block: @Sendable (DIContainer) -> Void) async {
    await DIContainer.shared.performBatchRegistration(block)
  }

  static func performBatchRegistration(_ block: @Sendable (DIContainer) async -> Void) async {
    await DIContainer.shared.performBatchRegistration(block)
  }
