...
public actor DIActor {
  ...
  public func resolve<T>(_ type: T.Type) async -> T? where T: Sendable {
    ...
    do {
      if let beginResolution = OptimizationHooks.beginResolution {
        try await beginResolution(type)
      }
    } catch {
#if DEBUG
...
#endif
      if let endResolution = OptimizationHooks.endResolution {
        await endResolution(type)
      }
      return nil
    }
