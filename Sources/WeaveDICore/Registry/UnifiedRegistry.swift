...
public actor UnifiedRegistry {
  ...
  /// KeyPath를 사용한 등록
  /// - Parameters:
  ///   - keyPath: DIContainer 내의 KeyPath
  ///   - factory: 인스턴스 생성 팩토리
    public func register<T>(
    keyPath: KeyPath<DIContainer, T?>,
    factory: @escaping @Sendable () -> T
  ) where T: Sendable {
  ...
  public func resolveAnyAsync(_ type: Any.Type) async -> Any? {
    ...
    if let (scopeKind, asyncFactory) = scopedAsyncFactories[key] {
      if let scopeId = ScopeContext.shared.currentID(for: scopeKind) {
        ...
        if let hook = OptimizationHooks.recordAutoEdgeIfEnabled { await hook(type) }
        return v.value
      }
    }
  ...
  public func resolveAsync<T>(_ type: T.Type) async -> T? where T: Sendable {
    ...
    if let fallbackResult = DIContainer.shared.resolve(type) {
  ...
  /// KeyPath를 사용한 해결 (async)
    public func resolveAsync<T>(keyPath: KeyPath<DIContainer, T?>) async -> T? where T: Sendable {
    let keyPathString = String(describing: keyPath)
