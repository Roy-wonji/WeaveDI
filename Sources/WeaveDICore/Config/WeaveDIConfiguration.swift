...
private struct WeaveDIConfigState {
  ...
  var monitorToggleHandler: (@Sendable (Bool) -> Void)?
...
public enum WeaveDIConfiguration {
  ...
  public static var enableAutoMonitor: Bool {
    ...
    set {
      let (previous, handler) = storage.withLock { state -> (Bool, (@Sendable (Bool) -> Void)?) in
        let oldValue = state.monitorEnabled
        state.monitorEnabled = newValue
        return (oldValue, state.monitorToggleHandler)
      }
      ...
      OptimizationHooks.setAutoMonitorEnabled?(flag)
      handler?(flag)
    }
  }

  /// Optional hook for monitoring modules to react to monitor toggles.
  public static func setMonitorToggleHandler(_ handler: (@Sendable (Bool) -> Void)?) {
    storage.withLock { state in
      state.monitorToggleHandler = handler
    }
  }
