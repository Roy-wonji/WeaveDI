...
#if swift(>=5.9)
@available(iOS 17.0, *)
/// `RegisterModule`은 Repository와 UseCase 모듈을 생성하고 의존성을 주입하는 공통 로직을 제공합니다.
/// 이 구조체를 통해 모듈을 생성하는 팩토리 메서드를 호출하여, DI 컨테이너에 의존성을 등록하거나
...
public struct RegisterModule: Sendable {
    ...
    /// 실제로는 `makeModule(_:factory:)`를 호출하여 `Module`을 생성합니다.
    ///
    /// - Parameters:
    ///   - type: 생성할 의존성의 타입
    ///   - factory: 의존성 인스턴스를 생성하는 클로저
    ...
    /// 통합 의존성 생성 함수로, 특정 프로토콜 타입에 대해 `Module`을 생성하는 클로저를 반환합니다.
    ///
    /// - Parameters:
    ///   - protocolType: 등록할 의존성의 프로토콜 타입
    ///   - factory: 의존성 인스턴스를 생성하는 클로저 (`U` 타입을 반환)
    /// - Returns: `Module`을 생성하는 클로저. 이 클로저를 호출하면 DI 컨테이너에 등록할 `Module`이 생성됩니다.
    public func makeDependency<T, U>(
        _ protocolType: T.Type,
        factory: @Sendable @escaping () -> U
    ) -> () -> Module {
        return {
            // 내부 헬퍼에서 factory()를 호출한 결과를 T 타입으로 캐스팅합니다.
            // 만약 캐스팅에 실패하면 런타임 오류가 발생합니다.
    ...
    /// Repository 의존성을 자동으로 주입받아 UseCase 모듈을 생성하는 함수입니다.
    ///
    /// - Parameters:
    ///   - useCaseProtocol: 등록할 UseCase 프로토콜 타입
    ///   - repositoryProtocol: 주입할 Repository 프로토콜 타입
    ///   - repositoryFallback: Repository가 등록되어 있지 않을 경우 반환할 기본 인스턴스를 생성하는 폐쇄형 클로저
    ///   - factory: 주입된 Repository를 사용하여 UseCase 인스턴스를 생성하는 클로저
    /// - Returns: `Module`을 생성하는 클로저. 호출 시 자동으로 Repository가 주입된 UseCase 모듈이 생성됩니다.
...
public struct RegisterModule {
    ...
    ///   - useCaseProtocol: 등록할 UseCase 프로토콜 타입
    ///   - repositoryProtocol: 주입할 Repository 프로토콜 타입
    ///   - repositoryFallback: Repository의 기본 인스턴스를 반환하는 클로저
    ///   - factory: 주입된 Repository를 사용하여 UseCase 인스턴스를 생성하는 클로저
    /// - Returns: `Module`을 생성하는 클로저
    public func makeUseCaseWithRepository<UseCase, Repo>(
        _ useCaseProtocol: UseCase.Type,
        repositoryProtocol: Repo.Type,
