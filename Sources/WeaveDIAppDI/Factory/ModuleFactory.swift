...
public struct ModuleFactoryManager: Sendable {
  ...
  /// 모든 모듈을 DI 컨테이너에 등록
  ///
  /// DIContainer에 정의된 registerAllDependencies()를 자동으로 호출합니다.
  ///
  /// ### 사용법:
  /// ```swift
  /// // 1. DIContainer에 의존성 정의
  ...
  /// ```
    public func registerAll(to container: DIContainer) async {
    // 1️⃣ DIContainer.registerAllDependencies() 자동 호출
    await DIContainer.registerAllDependencies()
    #logInfo("✅ DIContainer.registerAllDependencies() 호출 완료")
...
public extension ModuleFactoryManager {
  ...
  /// ```
    static func createAndRegisterAll(to container: DIContainer) async {
    let factory = ModuleFactoryManager()
    await factory.registerAll(to: container)
  }
}
