...
// MARK: - AutoSync Extension Macro

/// 🎯 **양방향 @AutoSyncExtension 매크로**: 한 줄로 TCA ↔ WeaveDI 완전 자동 동기화!
///
/// ## 사용법 (한 줄로 양방향 동기화):
/// ```swift
/// // TCA DependencyValues → WeaveDI 동기화
/// @AutoSyncExtension  // ← 이것만 추가하면 양방향 동기화!
/// extension DependencyValues {
...
/// @AutoSyncExtension  // ← InjectedValues도 지원!
...
/// ```
public struct AutoSyncExtensionMacro: MemberMacro {

  /// 🎯 사용자가 원하는 @AutoSyncExtension: extension 내 모든 computed property의 동기화 버전을 자동 생성
  public static func expansion(
    of node: AttributeSyntax,
    providingMembersOf declaration: some DeclGroupSyntax,
    in context: some MacroExpansionContext
...
// MARK: - AutoSyncExtensionLegacyMacro

/// TCA DependencyValues extension 전체에 멤버별 자동 동기화를 적용하는 매크로
public struct AutoSyncExtensionLegacyMacro: MemberAttributeMacro {
  public static func expansion(
    of node: AttributeSyntax,
    attachedTo declaration: some DeclGroupSyntax,
    providingAttributesFor member: some DeclSyntaxProtocol,
